...
Java_com_limelight_nvstream_jni_MoonBridge_startConnection(JNIEnv *env, jclass clazz,
                                                           ...
                                                           jintArray portMappingProtocols,
                                                           jintArray originalPorts,
                                                           jintArray mappedPorts) {
    LI_PORT_MAPPING portMappings[16];
    int portMappingCount = 0;
    jint* portMappingProtocolBuf = NULL;
    jint* originalPortBuf = NULL;
    jint* mappedPortBuf = NULL;

    if (portMappingProtocols != NULL && originalPorts != NULL && mappedPorts != NULL) {
        jsize protocolCount = (*env)->GetArrayLength(env, portMappingProtocols);
        jsize originalPortCount = (*env)->GetArrayLength(env, originalPorts);
        jsize mappedPortCount = (*env)->GetArrayLength(env, mappedPorts);
        jsize mappingCount = protocolCount;
        if (originalPortCount < mappingCount) {
            mappingCount = originalPortCount;
        }
        if (mappedPortCount < mappingCount) {
            mappingCount = mappedPortCount;
        }
        if (mappingCount > (jsize)(sizeof(portMappings) / sizeof(portMappings[0]))) {
            mappingCount = (jsize)(sizeof(portMappings) / sizeof(portMappings[0]));
        }

        portMappingProtocolBuf = (*env)->GetIntArrayElements(env, portMappingProtocols, NULL);
        originalPortBuf = (*env)->GetIntArrayElements(env, originalPorts, NULL);
        mappedPortBuf = (*env)->GetIntArrayElements(env, mappedPorts, NULL);

        if (portMappingProtocolBuf != NULL && originalPortBuf != NULL && mappedPortBuf != NULL) {
            for (jsize i = 0; i < mappingCount; i++) {
                if ((portMappingProtocolBuf[i] == LI_PORT_PROTO_TCP || portMappingProtocolBuf[i] == LI_PORT_PROTO_UDP) &&
                        originalPortBuf[i] > 0 && originalPortBuf[i] <= 65535 &&
                        mappedPortBuf[i] > 0 && mappedPortBuf[i] <= 65535) {
                    portMappings[portMappingCount].protocol = portMappingProtocolBuf[i];
                    portMappings[portMappingCount].originalPort = (unsigned short)originalPortBuf[i];
                    portMappings[portMappingCount].mappedPort = (unsigned short)mappedPortBuf[i];
                    portMappingCount++;
                }
            }
        }
    ...
    SERVER_INFORMATION serverInfo = {
            ...
            .portMappings = portMappingCount > 0 ? portMappings : NULL,
            .portMappingCount = portMappingCount,
    };
    ...
    if (portMappingProtocolBuf != NULL) {
        (*env)->ReleaseIntArrayElements(env, portMappingProtocols, portMappingProtocolBuf, JNI_ABORT);
    }
    if (originalPortBuf != NULL) {
        (*env)->ReleaseIntArrayElements(env, originalPorts, originalPortBuf, JNI_ABORT);
    }
    if (mappedPortBuf != NULL) {
        (*env)->ReleaseIntArrayElements(env, mappedPorts, mappedPortBuf, JNI_ABORT);
    }
