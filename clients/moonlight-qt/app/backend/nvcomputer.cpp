...
#define SER_NATDDNS_ENABLED "natddnsenabled"
#define SER_NATDDNS_URL "natddnsurl"
#define SER_NATDDNS_MAPPING "natddnsmapping"
...
NvComputer::NvComputer(QSettings& settings)
{
    ...
    this->natDdnsEnabled = settings.value(SER_NATDDNS_ENABLED).toBool();
    this->natDdnsUrl = settings.value(SER_NATDDNS_URL).toString();
    QByteArray natDdnsMappingJson = settings.value(SER_NATDDNS_MAPPING).toByteArray();
    if (!natDdnsMappingJson.isEmpty()) {
        try {
            this->natDdnsMapping = NatDdnsMapping::fromJson(natDdnsMappingJson, this->natDdnsUrl);
        } catch (const std::exception& e) {
            qWarning() << "Ignoring invalid NAT-DDNS mapping for" << this->name << ":" << e.what();
        }
    }
...
void NvComputer::serialize(QSettings& settings, bool serializeApps) const
{
    ...
    settings.setValue(SER_NATDDNS_ENABLED, natDdnsEnabled);
    settings.setValue(SER_NATDDNS_URL, natDdnsUrl);
    settings.setValue(SER_NATDDNS_MAPPING, natDdnsMapping.toJson());
...
bool NvComputer::isEqualSerialized(const NvComputer &that) const
{
    return this->name == that.name &&
           ...
           this->natDdnsEnabled == that.natDdnsEnabled &&
           this->natDdnsUrl == that.natDdnsUrl &&
           this->natDdnsMapping.toJson() == that.natDdnsMapping.toJson() &&
...
bool NvComputer::refreshNatDdnsMapping(QString* error)
{
    QString url;
    {
        QReadLocker readLocker(&lock);
    ...
    try {
        NatDdnsMapping mapping = NatDdnsResolver::fetch(url);

        QWriteLocker writeLocker(&lock);
        natDdnsMapping = mapping;

        quint16 activePort = activeAddress.port() != 0 ? activeAddress.port() : DEFAULT_HTTP_PORT;
        activeAddress = NvAddress(mapping.host, activePort);

        quint16 manualPort = manualAddress.port() != 0 ? manualAddress.port() : DEFAULT_HTTP_PORT;
        manualAddress = NvAddress(mapping.host, manualPort);

        if (!remoteAddress.isNull()) {
            remoteAddress = NvAddress(mapping.host, remoteAddress.port());
        }
...
#define ASSIGN_IF_CHANGED_AND_NONNULL(field)  \
    ...
    if (that.natDdnsEnabled) {
        ASSIGN_IF_CHANGED(natDdnsEnabled);
        ASSIGN_IF_CHANGED(natDdnsUrl);
        if (this->natDdnsMapping.toJson() != that.natDdnsMapping.toJson()) {
            this->natDdnsMapping = that.natDdnsMapping;
            changed = true;
        }
    }
