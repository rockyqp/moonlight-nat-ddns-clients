...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int* error) {
    ...
    *error = -1;
    ...
    // is listening.
    do {
        sock = connectTcpSocket(&RemoteAddr, AddrLen, LiGetMappedPort(LI_PORT_PROTO_TCP, RtspPortNumber), RTSP_CONNECT_TIMEOUT_SEC);
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    if (useEnet) {
        ...
        enet_address_set_port(&address, LiGetMappedPort(LI_PORT_PROTO_UDP, RtspPortNumber));
