...
int startControlStream(void) {
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        enet_address_set_port(&remoteAddress, LiGetMappedPort(LI_PORT_PROTO_UDP, ControlPortNumber));
        ...
        ctlSock = connectTcpSocket(&RemoteAddr, AddrLen,
            LiGetMappedPort(LI_PORT_PROTO_TCP, 47995), CONTROL_STREAM_TIMEOUT_SEC);
        if (ctlSock == INVALID_SOCKET) {
            stopping = true;
            return LastSocketFail();
        }
