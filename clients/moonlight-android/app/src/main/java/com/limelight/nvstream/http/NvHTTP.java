...
public class NvHTTP {
    ...
    private ComputerDetails.AddressTuple address;
    private NatDdnsMapping natDdnsMapping;
    ...
    public HttpUrl getHttpsUrl(boolean likelyOnline) throws IOException {
        ...
        return new HttpUrl.Builder().scheme("https").host(baseUrlHttp.host()).port(mapTcpPort(httpsPort)).build();
    }
    ...
    public NvHTTP(ComputerDetails.AddressTuple address, int httpsPort, String uniqueId, X509Certificate serverCert, LimelightCryptoProvider cryptoProvider) throws IOException {
        this(address, httpsPort, uniqueId, serverCert, cryptoProvider, NatDdnsMapping.empty());
    }

    public NvHTTP(ComputerDetails.AddressTuple address, int httpsPort, String uniqueId, X509Certificate serverCert,
                  LimelightCryptoProvider cryptoProvider, NatDdnsMapping natDdnsMapping) throws IOException {
        ...
        this.natDdnsMapping = natDdnsMapping != null ? natDdnsMapping : NatDdnsMapping.empty();
        ...
        try {
            ...
            String networkHost = this.natDdnsMapping.isValid() ? this.natDdnsMapping.host : addressString;
            this.baseUrlHttp = new HttpUrl.Builder()
                    .scheme("http")
                    .host(networkHost)
                    .port(mapTcpPort(address.port))
                    .build();
    ...
    private int mapTcpPort(int port) {
        if (natDdnsMapping == null || !natDdnsMapping.isValid()) {
            return port;
        }

        return natDdnsMapping.mapTcpPort(port);
    }
    ...
    public ComputerDetails getComputerDetails(String serverInfo) throws IOException, XmlPullParserException {
        ...
        details.localAddress = makeTuple(getXmlString(serverInfo, "LocalIP", false), address.port);
