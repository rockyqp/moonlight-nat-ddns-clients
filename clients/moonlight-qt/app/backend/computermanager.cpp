...
private:
    ...
    {
        NatDdnsMapping natDdnsMapping;
        {
            QReadLocker lock(&m_Computer->lock);
            natDdnsMapping = m_Computer->natDdnsMapping;
        }

        NvHTTP http(address, 0, m_Computer->serverCert, nam, natDdnsMapping);
...
private:
    void run()
    {
        QString natDdnsError;
        if (!m_Computer->refreshNatDdnsMapping(&natDdnsError)) {
            emit pairingCompleted(m_Computer, tr("Failed to refresh NAT-DDNS mapping: %1").arg(natDdnsError));
            return;
        }
...
private:
    void run()
    {
        QString natDdnsError;
        if (!m_Computer->refreshNatDdnsMapping(&natDdnsError)) {
            emit quitAppFailed(tr("Failed to refresh NAT-DDNS mapping: %1").arg(natDdnsError));
            return;
        }
...
void ComputerManager::addNewHostManually(QString address)
{
    ...
    QUrl natDdnsUrl(trimmedAddress);
    if (natDdnsUrl.isValid() &&
            !natDdnsUrl.host().isEmpty() &&
            (natDdnsUrl.scheme() == "http" || natDdnsUrl.scheme() == "https")) {
        addNewHost(NvAddress(), false, trimmedAddress);
        return;
    }

    QUrl url = QUrl::fromUserInput("moonlight://" + trimmedAddress);
    ...
    else if (QHostAddress(trimmedAddress).protocol() == QAbstractSocket::IPv6Protocol) {
        // The user specified an IPv6 literal without URL escaping, so use the default port
        addNewHost(NvAddress(trimmedAddress, DEFAULT_HTTP_PORT), false);
    }
    else {
        emit computerAddCompleted(false, false, QVariant());
    }
}
...
public:
    PendingAddTask(ComputerManager* computerManager, QString name, NvAddress address, NvAddress mdnsIpv6Address, bool mdns)
        : m_ComputerManager(computerManager),
          ...
          m_Mdns(mdns),
          m_NatDdns(!address.isNull() ? false : !name.isEmpty()),
          m_NatDdnsUrl(!address.isNull() ? QString() : name),
          m_AboutToQuit(false)
    {
...
signals:
    void computerAddCompleted(QVariant success, QVariant detectedPortBlocking, QVariant error);
...
private:
    ...
    void emitAddFailed(bool detectedPortBlocking, const QString& error = QString())
    {
        emit computerAddCompleted(false, detectedPortBlocking, error.isEmpty() ? QVariant() : error);
    }
    ...
    QString fetchServerInfo(NvHTTP& http)
    {
        ...
        try {
            ...
            requestError = e.toQString();
        } catch (const QtNetworkReplyException& e) {
            requestError = e.toQString();
        } catch (const std::exception& e) {
            requestError = QString::fromUtf8(e.what());
        } catch (...) {
            requestError = tr("Unknown error");
        }

        if (!m_Mdns) {
            unsigned int portTestResult;

            if (m_ComputerManager->m_Prefs->detectNetworkBlocking && !m_NatDdns) {
            ...
            QString error;
            if (m_NatDdns) {
                error = tr("NAT-DDNS mapping was fetched, but Moonlight could not connect to Sunshine at %1. Error: %2\n\n%3")
                        .arg(http.m_BaseUrlHttp.toString(), requestError, m_NatDdnsMapping.display());
            }
            emitAddFailed(portTestResult != 0 && portTestResult != ML_TEST_RESULT_INCONCLUSIVE, error);
        }
        return QString();
    }
    ...
    void run()
    {
        if (m_NatDdns) {
            try {
                m_NatDdnsMapping = NatDdnsResolver::fetch(m_NatDdnsUrl);
                if (!m_NatDdnsMapping.tcp.contains(DEFAULT_HTTP_PORT)) {
                    throw std::runtime_error("NAT-DDNS mapping is missing TCP 47989");
                }
                if (!m_NatDdnsMapping.tcp.contains(DEFAULT_HTTPS_PORT)) {
                    throw std::runtime_error("NAT-DDNS mapping is missing TCP 47984");
                }
                m_Address = NvAddress(m_NatDdnsMapping.host, DEFAULT_HTTP_PORT);
            } catch (const std::exception& e) {
                qWarning() << "Failed to fetch NAT-DDNS mapping:" << e.what();
                emitAddFailed(false, tr("Failed to fetch NAT-DDNS mapping: %1").arg(QString::fromUtf8(e.what())));
                return;
            }
        }

        NvHTTP http(m_Address, 0, QSslCertificate(), nullptr, m_NatDdnsMapping);
        ...
        if (m_NatDdns) {
            newComputer->natDdnsEnabled = true;
            newComputer->natDdnsUrl = m_NatDdnsUrl;
            newComputer->natDdnsMapping = m_NatDdnsMapping;
...

        {
            ...
            if (existingComputer != nullptr) {
                ...
                if (!m_Mdns) {
                    emit computerAddCompleted(true, false, QVariant());
                }
    ...
    bool m_Mdns;
    bool m_NatDdns;
    QString m_NatDdnsUrl;
    NatDdnsMapping m_NatDdnsMapping;
    bool m_AboutToQuit;
};
...
class PendingNatDdnsRefreshTask : public QObject, public QRunnable
{
    Q_OBJECT

public:
    PendingNatDdnsRefreshTask(ComputerManager* computerManager, NvComputer* computer)
        : m_Computer(computer)
    {
        connect(this, &PendingNatDdnsRefreshTask::refreshCompleted,
                computerManager, &ComputerManager::handleNatDdnsRefreshCompleted);
    }

signals:
    void refreshCompleted(NvComputer* computer, QString error);
...
private:
    void run()
    {
        QString error;
        if (!m_Computer->refreshNatDdnsMapping(&error)) {
            emit refreshCompleted(m_Computer, error);
            return;
        }

        emit refreshCompleted(m_Computer, QString());
    }

    NvComputer* m_Computer;
};

void ComputerManager::refreshNatDdnsHost(NvComputer* computer)
{
    QReadLocker lock(&computer->lock);
    if (!computer->natDdnsEnabled) {
        return;
    }
    lock.unlock();

    PendingNatDdnsRefreshTask* refresh = new PendingNatDdnsRefreshTask(this, computer);
    QThreadPool::globalInstance()->start(refresh);
}

void ComputerManager::handleNatDdnsRefreshCompleted(NvComputer* computer, QString error)
{
    if (error.isEmpty()) {
        handleComputerStateChanged(computer);
    }

    emit natDdnsRefreshCompleted(computer, error);
}
