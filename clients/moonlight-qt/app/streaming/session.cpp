...
bool Session::startConnectionAsync()
{
    ...
    try {
        ...
        if (!m_Computer->refreshNatDdnsMapping(&natDdnsError)) {
            emit displayLaunchError(tr("Failed to refresh NAT-DDNS mapping: %1").arg(natDdnsError));
            return false;
        }
    ...
    int portMappingCount = 0;
    {
        ...
        if (m_Computer->natDdnsEnabled && m_Computer->natDdnsMapping.isValid()) {
            for (auto it = m_Computer->natDdnsMapping.tcp.constBegin();
                 it != m_Computer->natDdnsMapping.tcp.constEnd() && portMappingCount < (int)(sizeof(portMappings) / sizeof(portMappings[0]));
                 ++it) {
                portMappings[portMappingCount].protocol = LI_PORT_PROTO_TCP;
                portMappings[portMappingCount].originalPort = it.key();
                portMappings[portMappingCount].mappedPort = it.value();
                portMappingCount++;
            }
            for (auto it = m_Computer->natDdnsMapping.udp.constBegin();
                 it != m_Computer->natDdnsMapping.udp.constEnd() && portMappingCount < (int)(sizeof(portMappings) / sizeof(portMappings[0]));
                 ++it) {
                portMappings[portMappingCount].protocol = LI_PORT_PROTO_UDP;
                portMappings[portMappingCount].originalPort = it.key();
                portMappings[portMappingCount].mappedPort = it.value();
                portMappingCount++;
            }
    ...
    if (portMappingCount > 0) {
        hostInfo.portMappings = portMappings;
        hostInfo.portMappingCount = portMappingCount;
    }
