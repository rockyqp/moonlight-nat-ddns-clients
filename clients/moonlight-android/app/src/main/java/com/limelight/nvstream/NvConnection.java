...
import com.limelight.nvstream.http.NatDdnsMapping;
import com.limelight.nvstream.http.NatDdnsResolver;
...
public class NvConnection {
    ...
    {
        this(appContext, host, httpsPort, uniqueId, config, cryptoProvider, serverCert, NatDdnsMapping.empty());
    }

    public NvConnection(Context appContext, ComputerDetails.AddressTuple host, int httpsPort, String uniqueId, StreamConfiguration config,
                        LimelightCryptoProvider cryptoProvider, X509Certificate serverCert, NatDdnsMapping natDdnsMapping)
    {
        ...
        this.context.natDdnsMapping = natDdnsMapping != null ? natDdnsMapping : NatDdnsMapping.empty();
    ...
    {
        if (context.natDdnsMapping != null && context.natDdnsMapping.isValid() &&
                context.natDdnsMapping.sourceUrl != null) {
            context.natDdnsMapping = NatDdnsResolver.fetch(context.natDdnsMapping.sourceUrl);
            context.serverAddress = new ComputerDetails.AddressTuple(
                    context.natDdnsMapping.host,
                    context.serverAddress.port);
        }

        NvHTTP h = new NvHTTP(context.serverAddress, context.httpsPort, uniqueId, context.serverCert,
                cryptoProvider, context.natDdnsMapping);
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                synchronized (MoonBridge.class) {
                    ...
                    int[] portMappingProtocols = context.natDdnsMapping.buildProtocolArray();
                    int[] originalPorts = context.natDdnsMapping.buildOriginalPortArray();
                    int[] mappedPorts = context.natDdnsMapping.buildMappedPortArray();
