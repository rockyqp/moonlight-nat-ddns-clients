...
public class ComputerManagerService extends Service {
    ...
    private ComputerDetails tryPollIp(ComputerDetails details, ComputerDetails.AddressTuple address) {
        try {
            ...
            NvHTTP http = new NvHTTP(address, portMatchesActiveAddress ? details.httpsPort : 0, idManager.getUniqueId(), details.serverCert,
                    PlatformBinding.getCryptoProvider(ComputerManagerService.this), details.natDdnsMapping);
            ...
            newDetails.copyNatDdnsFrom(details);
    ...
    private boolean pollComputer(ComputerDetails details) throws InterruptedException {
        StringBuilder natDdnsError = new StringBuilder();
        if (!details.refreshNatDdnsMapping(natDdnsError)) {
            LimeLog.warning("NAT-DDNS refresh failed for " + details.name + ": " + natDdnsError);
            return false;
        }
            ...
            thread = new Thread() {
                ...
                public void run() {
                    ...
                    do {
                        ...
                        try {
                            StringBuilder natDdnsError = new StringBuilder();
                            if (!computer.refreshNatDdnsMapping(natDdnsError)) {
                                throw new IOException("Failed to refresh NAT-DDNS mapping: " + natDdnsError);
                            }

                            NvHTTP http = new NvHTTP(ServerHelper.getCurrentAddressFromComputer(computer), computer.httpsPort, idManager.getUniqueId(),
                                    computer.serverCert, PlatformBinding.getCryptoProvider(ComputerManagerService.this), computer.natDdnsMapping);
