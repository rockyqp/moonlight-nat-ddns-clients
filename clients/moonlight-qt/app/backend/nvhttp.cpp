...
NvHTTP::NvHTTP(NvAddress address,
               uint16_t httpsPort,
    ...
    m_NatDdnsMapping(natDdnsMapping),
    m_HttpsPort(0)
{
...
NvHTTP::NvHTTP(NvComputer* computer, QNetworkAccessManager* nam) :
    NvHTTP(computer->activeAddress,
           computer->activeHttpsPort,
           computer->serverCert,
           nam,
           computer->natDdnsMapping)
{
...
void NvHTTP::setAddress(NvAddress address)
{
    ...
    QString networkHost = m_NatDdnsMapping.isValid() ? m_NatDdnsMapping.host : address.address();
    m_BaseUrlHttp.setHost(networkHost);
    m_BaseUrlHttps.setHost(networkHost);

    m_BaseUrlHttp.setPort(mapTcpPort(address.port()));
}

void NvHTTP::setHttpsPort(uint16_t port)
{
    m_HttpsPort = port;
    m_BaseUrlHttps.setPort(mapTcpPort(port));
}

void NvHTTP::setNatDdnsMapping(NatDdnsMapping natDdnsMapping)
{
    m_NatDdnsMapping = natDdnsMapping;
    if (!m_Address.isNull()) {
        setAddress(m_Address);
    }
    setHttpsPort(m_HttpsPort);
}

NvAddress NvHTTP::address()
{
    if (m_NatDdnsMapping.isValid()) {
        return NvAddress(m_NatDdnsMapping.host, m_Address.port());
    }
...
uint16_t NvHTTP::mapTcpPort(uint16_t port) const
{
    if (port == 0 || !m_NatDdnsMapping.isValid()) {
        return port;
    }

    return m_NatDdnsMapping.mapTcpPort(port);
}
