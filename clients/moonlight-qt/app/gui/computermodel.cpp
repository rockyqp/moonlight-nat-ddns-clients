...
void ComputerModel::initialize(ComputerManager* computerManager)
{
    ...
    connect(m_ComputerManager, &ComputerManager::natDdnsRefreshCompleted,
            this, &ComputerModel::handleNatDdnsRefreshCompleted);
...
QVariant ComputerModel::data(const QModelIndex& index, int role) const
{
    ...
    case NatDdnsRole:
        return computer->natDdnsEnabled;
    ...
    case DetailsRole: {
        ...
        return tr("Name: %1").arg(computer->name) + '\n' +
               ...
               (computer->natDdnsEnabled ? (QStringLiteral("\n") + computer->natDdnsMapping.display()) : QString());
    }
    default:
        return QVariant();
    }
...
void ComputerModel::refreshNatDdnsForComputer(int computerIndex)
{
    Q_ASSERT(computerIndex < m_Computers.count());

    m_ComputerManager->refreshNatDdnsHost(m_Computers[computerIndex]);
}
...
void ComputerModel::handleNatDdnsRefreshCompleted(NvComputer*, QString error)
{
    emit natDdnsRefreshCompleted(error.isEmpty() ? QVariant() : error);
}
