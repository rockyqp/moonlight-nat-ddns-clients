...
unsigned short LiGetMappedPort(int protocol, unsigned short port) {
    int i;

    for (i = 0; i < PortMappingCount; i++) {
        if (PortMappings[i].protocol == protocol && PortMappings[i].originalPort == port) {
            return PortMappings[i].mappedPort;
        }
    }
...
int LiStartConnection(PSERVER_INFORMATION serverInfo, PSTREAM_CONFIGURATION streamConfig, PCONNECTION_LISTENER_CALLBACKS clCallbacks,
    ...
    PortMappingCount = 0;
    if (serverInfo->portMappings != NULL && serverInfo->portMappingCount > 0) {
        int mappingCount = serverInfo->portMappingCount;
        if (mappingCount > (int)(sizeof(PortMappings) / sizeof(PortMappings[0]))) {
            mappingCount = (int)(sizeof(PortMappings) / sizeof(PortMappings[0]));
        }
        memcpy(PortMappings, serverInfo->portMappings, mappingCount * sizeof(PortMappings[0]));
        PortMappingCount = mappingCount;
    }
    else {
        Limelog("RTSP port: %u\n", RtspPortNumber);
    }
    ...
    if (RtspPortNumber != 48010) {
        ...
        err = resolveHostName(serverInfo->address, AF_UNSPEC, LiGetMappedPort(LI_PORT_PROTO_TCP, RtspPortNumber), &RemoteAddr, &AddrLen);
        if (err != 0) {
        ...
        err = resolveHostName(serverInfo->address, AF_UNSPEC, LiGetMappedPort(LI_PORT_PROTO_TCP, 47984), &RemoteAddr, &AddrLen);
        if (err != 0) {
            err = resolveHostName(serverInfo->address, AF_UNSPEC, LiGetMappedPort(LI_PORT_PROTO_TCP, 47989), &RemoteAddr, &AddrLen);
        }
        if (err != 0) {
            err = resolveHostName(serverInfo->address, AF_UNSPEC, LiGetMappedPort(LI_PORT_PROTO_TCP, 48010), &RemoteAddr, &AddrLen);
        }
    }
    if (err != 0) {
        Limelog("failed: %d\n", err);
    ...
    // Resolve LocalAddr by RemoteAddr.
    {
        ...
        err = getLocalAddressByUdpConnect(&RemoteAddr, AddrLen, LiGetMappedPort(LI_PORT_PROTO_TCP, RtspPortNumber), &LocalAddr, &localAddrLen);
        if (err != 0) {
